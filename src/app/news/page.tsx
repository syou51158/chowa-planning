...
export default async function NewsPage({ searchParams }: NewsPageProps) {
  ...
  if (resolvedSearchParams.year) {
    filteredNews = filteredNews.filter(post => {
      try {
        return new Date(post.publishedAt).getFullYear().toString() === resolvedSearchParams.year;
      } catch {
        return false;
      }
    });
    ...
    <div className="min-h-screen bg-brand-natural/30">
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          ...
          >
            <Card className="border-0 shadow-lg rounded-2xl bg-brand-primary text-white">
              <CardContent className="p-6 text-center">
                <Newspaper className="w-8 h-8 mx-auto mb-3 text-brand-accent" />
                <div className="text-3xl font-bold mb-2">{news.length}</div>
                <div className="text-brand-concrete">総記事数</div>
              </CardContent>
            </Card>

            <Card className="border-0 shadow-lg rounded-2xl bg-brand-secondary text-white">
              <CardContent className="p-6 text-center">
            ...
            <Card className="border-0 shadow-lg rounded-2xl bg-brand-steel text-white">
              <CardContent className="p-6 text-center">
                <Calendar className="w-8 h-8 mx-auto mb-3 text-brand-accent" />
                <div className="text-3xl font-bold mb-2">
                  {news.length > 0 ? format(new Date(news[0].publishedAt), 'M月', { locale: ja }) : '-'}
                </div>
        ...
        <div className="container mx-auto px-4">
          ...
          >
            ...
            {categories.map((category) => (
              ...
              >
                <Link
                  href={`/news?category=${encodeURIComponent(category)}${currentYear ? `&year=${currentYear}` : ''
                    }`}
                >
                  {category}
                </Link>
            ...
            {years.slice(0, 5).map((year) => (
              ...
              >
                <Link
                  href={`/news?year=${year}${currentCategory ? `&category=${encodeURIComponent(currentCategory)}` : ''
                    }`}
                >
                  {year}年
                </Link>
